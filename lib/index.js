...
function Metalsmith(directory){
  ...
  this.except([]);
  this._flags = args(process.argv.slice(2));
  this.verbose = (this._flags.verbose || this._flags.v) ? true : false;
  if (this.verbose) {
    console.log('Metalsmith: Running in verbose mode.');
  }
}
...
Metalsmith.prototype.except = function(except){
  if (!arguments.length) return this._except;
  this._except = except;
  return this;
};
...
Metalsmith.prototype.build = unyield(function*(){
  ...
  // readdir is async, that's why we need those "multiple" each(Object.keys(files), wri
  if (except.length === 0) {
    if (clean) rm(path.join(dest, '*'));

    if (this.verbose && clean) {
      console.log('\t...cleaned...');
    }
    ...
    files = yield this.run(files);   // run+read = process??
    if (this.verbose) {
    ...
    if (clean) {
      var files = yield this.read();
      readdir(dest, except, function(error, files) {
  ...
  try {
    ...
    if (frontmatter && utf8(buffer)) {
      try {
        ...
        parsed = matter(stripped.toString());
      } catch (e) {
